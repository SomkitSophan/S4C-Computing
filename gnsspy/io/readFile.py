...
def read_navFile(navigationFile):
    ...
    while True:
        ...
        elif version.startswith("2") & ('ION ALPHA' in nav[line]):
            temp = "".join([lines.replace('D', 'E') for lines in nav[line][0:-21]])
            Iono[0] = float(temp[0:15])
            Iono[1] = float(temp[16:27])
            Iono[2] = float(temp[28:39])
            Iono[3] = float(temp[40:])
            line += 1
        elif version.startswith("3") & ('GPSA' in nav[line]) & ('IONOSPHERIC CORR' in nav[line]):
            temp = "".join([lines.replace('D', 'E') for lines in nav[line][4:-21]])
            Iono[0] = float(temp[0:14])
            Iono[1] = float(temp[15:26])
            Iono[2] = float(temp[27:38])
            Iono[3] = float(temp[39:])
            line += 1
        elif version.startswith("2") & ('ION BETA' in nav[line]):
            temp = "".join([lines.replace('D', 'E') for lines in nav[line][0:-21]])
            Iono[4] = float(temp[0:15])
            Iono[5] = float(temp[16:27])
            Iono[6] = float(temp[28:39])
            Iono[7] = float(temp[40:])
            line += 1
        elif version.startswith("3") & ('GPSB' in nav[line]) & ('IONOSPHERIC CORR' in nav[line]):
            temp = "".join([lines.replace('D', 'E') for lines in nav[line][4:-21]])
            Iono[4] = float(temp[0:14])
            Iono[5] = float(temp[15:26])
            Iono[6] = float(temp[27:38])
            Iono[7] = float(temp[39:])
            line += 1
        elif version.startswith("2") & ('DELTA-UTC' in nav[line]):
            temp = "".join([lines.replace('D', 'E') for lines in nav[line][0:-21]])
            dUTC = [[float(temp[0:23]), float(temp[24:42]), float(temp[43:51]), float(temp[52:])]]
            line += 1
        elif version.startswith("3") & ('GPUT' in nav[line]) & ('TIME SYSTEM CORR' in nav[line]):
            temp = "".join([lines.replace('D', 'E') for lines in nav[line][4:-21]])
            dUTC = [[float(temp[0:18]), float(temp[19:34]), float(temp[35:41]), float(temp[42:])]]
            line += 1
    ...
    while True:
        ...
        epoch = datetime.datetime(year=year, month=month, day=day, hour=hour, minute=minute, second=second)
        # --------------------------------------------------------------
        af0 = float(nav[0][7])
        af1 = float(nav[0][8])
        af2 = float(nav[0][9])
        if GLONASS or SBAS:
            X, Xdot, Xacc, SVh = float(nav[1][0]), float(nav[1][1]), float(nav[1][2]), float(nav[1][3])
            Y, Ydot, Yacc, flagP2 = float(nav[2][0]), float(nav[2][1]), float(nav[2][2]), float(nav[2][3])
            Z, Zdot, Zacc, dT = float(nav[3][0]), float(nav[3][1]), float(nav[3][2]), float(nav[3][3])
            IODE, crs, delta_n, M0, cuc, e, cus, sqrtA, Toe, cic, OMEGA0, cis, I0, crc, omega, OMEGAdot,\
            Idot, code2, SOW, TGD, IODC, SVa, Ts = [np.nan for _ in range(23)]

            if PRN == PRN_old and epoch == epoch_old:
                ephemeris_list[-1] = [[PRN, epoch, af0, af1, af2,
                                      IODE, crs, delta_n, M0,
                                      cuc, e, cus, sqrtA,
                                      Toe, cic, OMEGA0, cis,
                                      I0, crc, omega, OMEGAdot,
                                      Idot, code2, SOW, flagP2,
                                      SVa, SVh, TGD, IODC, Ts, dT,
                                      X, Y, Z, Xdot, Ydot, Zdot, Xacc, Yacc, Zacc]]
            else:
                svList.append(PRN)
                ephemeris_list.append([PRN, epoch, af0, af1, af2,
                                      IODE, crs, delta_n, M0,
                                      cuc, e, cus, sqrtA,
                                      Toe, cic, OMEGA0, cis,
                                      I0, crc, omega, OMEGAdot,
            ...
            Toe = float(nav[3][0])
            cic = float(nav[3][1])
            OMEGA0 = float(nav[3][2])
            cis = float(nav[3][3])
            # ---
            I0 = float(nav[4][0])
            crc = float(nav[4][1])
            omega = float(nav[4][2])
            OMEGAdot = float(nav[4][3])
            # ---
            Idot = float(nav[5][0])
            code2 = float(nav[5][1])
            SOW = float(nav[5][2])
            flagP2 = float(nav[5][3])
            # ---
            SVa = float(nav[6][0])
            SVh = float(nav[6][1])
            TGD = float(nav[6][2])
            ...
            Ts = float(nav[7][0])
            dT = float(nav[7][1])
            X, Y, Z, Xdot, Ydot, Zdot, Xacc, Yacc, Zacc = [np.nan for _ in range(9)]
    ...
    columnNames = ["SV", "epoch", "af0", "af1", "af2", "IODE", "crs", "delta_n", "M0", "cuc",
                   "e", "cus", "sqrtA", "Toe", "cic", "OMEGA0", "cis", "I0", "crc", "omega",
                   "OMEGAdot", "Idot", "code2", "SOW", "flagP2", "SVa", "SVh", "TGD", "IODC",
                   "Ts", "dT", "X", "Y", "Z", "Xdot", "Ydot", "Zdot", "Xacc", "Yacc", "Zacc"]
    ...
    return Navigation(ephemeris, Iono, dUTC, version)
...
def read_obsFile_v2(observationFile):
    ...
    while True:
        if 'RINEX VERSION / TYPE' in obsLines[line]:
            ...
            time_first = obsLines[line][0:obsLines[line].index('G')].split()
            time_first = [int(float(i)) for i in time_first]
            line += 1
        elif 'TIME OF LAST OBS' in obsLines[line]:
            time_last = obsLines[line][0:obsLines[line].index('G')].split()
            time_last = [int(float(i)) for i in time_last]
            line += 1
    ...
    return Observation(f.name, fileEpoch, observation, approx_position, receiver_type, antenna_type, interval, receiver_clock, version, ToB, time_first, time_last)
...
def read_obsFile_v3(obsFileName):
    ...
    return Observation(f.name, fileEpoch, obs, approx_position, receiver_type, antenna_type, interval, receiver_clock, version, observation_types, time_first, time_last)
